...
<div class="page-content">
    <div class="container-fluid">
        ...
        @php
            $isEdit = $bill !== null;
            $action = $isEdit ? url('admin/accounting/purchases/update/'.$bill->id) : url('admin/accounting/purchases/store');
            $oldItems = old('items');
            if (! $oldItems && $isEdit) {
                $oldItems = $bill->items->map(fn ($i) => [
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header bg-white border-bottom">
                        <h4 class="card-title mb-0"><i class="uil uil-receipt me-1"></i> {{ $title }}</h4>
                    </div>
                    <div class="card-body">
                        <form action="{{ $action }}" method="POST" id="billForm">
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select name="contact_id" class="form-select" required>
                                        ...
                                        @foreach ($contacts as $c)
                                            <option value="{{ $c->id }}" {{ old('contact_id', $isEdit ? $bill->contact_id : '') == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-3 mb-3">
                                    <label class="form-label fw-bold">Tanggal</label>
                                    <input type="date" name="date" class="form-control" value="{{ old('date', $isEdit ? $bill->date->toDateString() : now()->toDateString()) }}" required>
                                </div>
                                <div class="col-md-3 mb-3">
                                    <label class="form-label">Jatuh Tempo</label>
                                    <input type="date" name="due_date" class="form-control" value="{{ old('due_date', $isEdit && $bill->due_date ? $bill->due_date->toDateString() : '') }}">
                                </div>
                            </div>
                                ...
                                <table class="table table-bordered" id="itemsTable">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th style="width:15%;">Akun Beban/Aset</th>
                                                ...
                                                <td>
                                                    <select name="items[{{ $i }}][product_id]" class="form-select form-select-sm prod-select">
                                                        ...
                                                        @foreach ($products as $p)
                                                            <option value="{{ $p->id }}" data-price="{{ $p->purchase_price }}" data-account="{{ $p->expense_account_id }}" data-name="{{ $p->name }}" {{ ($item['product_id'] ?? '') == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                            ...
                            <div class="row">
                                <div class="col-md-7">
                                    ...
                                    <textarea name="notes" class="form-control" rows="3">{{ old('notes', $isEdit ? $bill->notes : '') }}</textarea>
                                    <label class="form-label mt-2">Referensi</label>
                                    <input type="text" name="reference" class="form-control" value="{{ old('reference', $isEdit ? $bill->reference : '') }}">
                                </div>
                                <div class="col-md-5">
                                    <table class="table table-sm">
                                        <tr><th>Subtotal</th><td class="text-end" id="subtotalCell">0</td></tr>
                                        <tr><th>Diskon</th><td><input type="number" step="0.01" name="discount" id="discountIn" class="form-control form-control-sm text-end" value="{{ old('discount', $isEdit ? $bill->discount : 0) }}"></td></tr>
                                        <tr><th>Pajak (PPN)</th><td><input type="number" step="0.01" name="tax" id="taxIn" class="form-control form-control-sm text-end" value="{{ old('tax', $isEdit ? $bill->tax : 0) }}"></td></tr>
                            ...
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn btn-primary"><i class="uil uil-save"></i> Simpan Tagihan</button>
                                <a href="{{ url('admin/accounting/purchases') }}" class="btn btn-secondary">Batal</a>
                            </div>
                        </form>
                    </div>
                </div>
